...
/** Everything below the title/export row on dashboard/page.tsx --
 * extracted so the Puppeteer-printed twin (print/dashboard/page.tsx)
 * renders identical markup, same pattern as WorkspaceOverviewMain.tsx.
 * Takes `data` as a prop rather than fetching it internally (unlike
 * WorkspaceOverviewMain) because the real page also needs the same
 * object for its CSV ExportButton -- fetching twice would mean the CSV
 * and the on-screen numbers could theoretically read different rows if
 * something changed between the two calls. */
export function CrossSpaceDashboardMain({ data }: { data: CrossSpaceDashboard }) {
  return (
    <>
      <Card>
        ...
        {data.handoverQueue.length === 0 ? (
          ...
          data.handoverQueue.map((p) => (
            <div key={p.ref} className="flex items-center gap-2.5 px-4 py-[11px] border-b border-line-soft last:border-b-0 text-[12px]">
              ...
              <LinkButton href={`/missions/projects/${p.ref.toLowerCase()}`} variant="coral">
                Open service
              </LinkButton>
            </div>
          ))

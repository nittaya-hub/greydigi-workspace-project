 ...
 * tab and the workspace-wide /missions/tasks view — so both need
 * revalidating together or one of them serves stale data after an edit
 * made from the other. */
function revalidateTaskViews(projectRef: string) {
  revalidatePath(`/missions/projects/${projectRef.toLowerCase()}/tasks`);
  revalidatePath("/missions/tasks");
}
  ...
  if (actor) {
    ...
    if (assigneePersonId && assigneePersonId !== actor.id) {
      await supabase.from("notifications").insert({
        ...
        related_url: `/missions/projects/${projectRef.toLowerCase()}/tasks?task=${taskId}`,
      });
    }
  }
...
export async function addTaskComment(taskId: string, projectRef: string, body: string) {
  ...
  await notifyWorkspace(
    actor.workspace_id,
    {
      ...
      relatedUrl: `/missions/projects/${projectRef.toLowerCase()}/tasks?task=${taskId}`,
    },

  ...
  if (!workspaceId) {
    return (
      <div className="px-4 py-5 sm:px-7 sm:py-8 flex flex-col gap-6 max-w-[1400px] mx-auto">
        <PageHeading title="Hangar" description="Accelerators and internal capability." />
        <Card className="p-5">
  ...
  return (
    <div className="px-4 py-5 sm:px-7 sm:py-8 flex flex-col gap-6 max-w-[1400px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-5">
        <PageHeading
          title="Hangar"
          description="Accelerators and internal capability. Missions projects depend on releases from here, and those dependencies are explicit."
        />
        <div className="flex gap-1.5 flex-none">
          ...
          />
          <ExportPdfButton href="/hangar/pdf" fallbackFilename={`product-features-${new Date().toISOString().slice(0, 10)}.pdf`} />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        />
        <StatTile
          label="MISSIONS BLOCKED BY HANGAR"
            ...
            <>
              ...
              {overview.features.map((f, i) => (
                <TableRow cols={FEAT_COLS} key={f.ref} last={i === overview.features.length - 1}>
                  ...
                  <Link href={`/hangar/features/${f.ref.toLowerCase()}`}>
          ...
          {nextRelease ? (
            <HeroPanel>
              ...
              <span className="text-[11.5px] text-muted-2 leading-[1.5]">
                Readiness is met criteria over required criteria. Same rule as gate completion in missions.
              </span>
            </HeroPanel>
          ) : null}

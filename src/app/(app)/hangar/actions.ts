...
export async function createFeature(formData: FormData) {
  ...
  await notifyWorkspace(
    product.workspace_id,
    {
      ...
      relatedUrl: `/hangar/features/${ref.toLowerCase()}`,
    },
    { excludePersonId: person?.id }
  );

  revalidatePath("/hangar");
  revalidatePath("/hangar/features");
  revalidatePath("/hangar/roadmap");
  revalidatePath("/");
}
...
export async function toggleFeatureClientVisible(itemId: string, itemRef: string, nextValue: boolean) {
  ...
  await notifyWorkspace(
    product.workspace_id,
    {
      ...
      relatedUrl: `/hangar/features/${itemRef.toLowerCase()}`,
    },
    { excludePersonId: person?.id }
  );

  revalidatePath("/hangar/features");
  revalidatePath(`/hangar/features/${itemRef.toLowerCase()}`);
  revalidatePath("/hangar/roadmap");
}

-- Public share-link submissions (fn_public_submit_client_submission,
-- fn_public_submit_hypercare_submission, both 0033/0046) insert into
-- client_submissions correctly, but the Node-side notifyWorkspace() call
-- that followed each one ran through the anonymous visitor's own
-- session -- and people_read (0007_rls.sql) and notifications_insert_
-- internal (0011_notifications_rls.sql) both key off
-- fn_my_internal_workspace_ids(), which is empty with no authenticated
-- user. notifyWorkspace's own read of `people` silently came back with
-- zero rows, so it returned early and inserted nothing. Testing a
-- public link's submission forms produced a real client_submissions row
-- every time, but never a notification -- nothing on the bell, ever.
--
-- Fix: notify from inside these two security-definer functions instead,
-- the same way they already bypass RLS to insert the submission itself.
-- `kind` is tagged with where the submission came from
-- (delivery_submission_* for the project share link, hypercare_
-- submission_* for the weekly report link) so the notifications list
-- can group by source and flag both as needing action -- see the
-- matching src/app/(app)/notifications/page.tsx change.
